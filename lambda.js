...
export async function handler(event) {
  ...
  if (!code || !state) {
    return {
      ...
      body: JSON.stringify({
        message: 'Missing authorization code or state parameter in body.',
      }),
    };
  }
  ...
  try {
        ...
        (res) => {
          ...
          res.on('end', () => {
            if (res.statusCode === 200) {
              ...
              console.error(
                `Bungie API token exchange failed with status ${res.statusCode}. Body: ${data}`,
              );
              reject(
                new Error(
                  `Bungie API token exchange failed: ${res.statusCode}. Detail: ${data}`,
                ),
              );
            }
          });
          ...
          res.on('end', () => {
            if (res.statusCode === 200) {
              try {
                ...
                reject(
                  new Error('Failed to parse linked profiles response.', e),
                );
              }
            } else {
              // Log the full error from Bungie's API
              console.error(
                `Linked profiles lookup failed with status ${res.statusCode}. Body: ${data}`,
              );
              reject(
                new Error(
                  `Linked profiles lookup failed: ${res.statusCode}. Detail: ${data}`,
                ),
              );
            }
          });
    ...
    const destinyProfile = linkedProfilesResponse.Response.profiles?.find(
      (p) => p.membershipType !== 254,
    );
    const destinyMembershipType = destinyProfile
